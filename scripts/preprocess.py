...
# Converts hold coordinates to a dict key
    ...
    for problem_id in problems:
        ...
        for section in ['start', 'mid', 'end']:
            for hold_coord in problems[problem_id][section]:
                hold = coord_to_key(hold_coord)

                # Initialises hold frequency array for that hold if it does not already exist
                if hold not in hold_freq:
                    hold_freq[hold] = [0 for i in range(18)]

                hold_freq[hold][grade] += 1
...
def move_to_closest_holds(holds, beta):
    ...
    while len(used_hold_dict) < len(holds):
        ...
        last_coord_1 = key_to_coord(last_move_for_hand_1[0])

        last_move_for_hand_2 = beta[-2]
        last_coord_2 = key_to_coord(last_move_for_hand_2[0])
        ...
        for i in range(len(holds)):
            ...
            if curr_hold not in used_hold_dict:
                coords = holds[i]
...
def move_to_match_on_final_hold(coords, beta):
    for i in range(2):
        ...
        last_coord = key_to_coord(last_move_for_hand[0])
        ...
        curr_angle /= 180
        beta.append(
            (coord_to_key(coords), weighted_distance, curr_angle))

    return beta


# TODO: Simplify keys for holds, potentially with tuples as the key. No more conversions
            ...
            if len(problem['start']) == 2:
                ...
                if problem['start'][1] == problem['start'][1][1]:
                    ...
                    if problem['start'][0][0] < problem['start'][1][0]:
                        beta.append((coord_to_key(problem['start'][0]), 0, 0))
                        beta.append((coord_to_key(problem['start'][1]), 0, 0))
                    else:
                        beta.append((coord_to_key(problem['start'][1]), 0, 0))
                        beta.append((coord_to_key(problem['start'][0]), 0, 0))
